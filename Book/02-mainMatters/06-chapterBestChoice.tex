...
As we observe indeed no such chordless circuits here, we may directly compute the \emph{prekernels}\index{showPreKernels@\texttt{showPreKernels()}} Of the outranking digraph $g$.
...
This result is shown with the \texttt{showBestChoiceRecommendation()}\index{showBestChoiceRecommendation@\texttt{showBestChoiceRecommendation()}} method. Notice that this method actually works by default on the broken chords digraph $bcg$. Therefore we set the \texttt{CoDual} flag to \texttt{False} (see Line 1 in Listing \ref{list:6.4} below).
...
To model these \emph{strict outranking} situations\index{outranking!strict}, we may recompute the best choice recommendation on the \emph{codual}\index{codual digraph}, the converse ($\sim$) of the dual ($-$)\footnote{Not to be confused with the dual graph of a plane graph $g$ that has a vertex for each face of $g$. Here we mean the \emph{less than} (strict converse) relation corresponding to a \emph{greater or equal} relation, or the \emph{less than or equal} relation corresponding to a (strict) \emph{better than} relation.}, of the outranking digraph instance $g$ (see \citet{BIS-2013}]), as follows:
