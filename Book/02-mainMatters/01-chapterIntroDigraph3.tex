...
\abstract*{ The chapter is devoted to a first contact with the \Digraph Python resources. Following installation instructions, we start a first Python session for generating, saving and inspecting a random crisp digraph.}
...
In the \Digraph resources, all digraph object instances are of \texttt{Digraph}\index{Digraph@\texttt{Digraph} class} type and contain at least the following attributes (see Listing~\vref{list:1.1}  Lines 11-12):
