...
\end{verbatim}
\end{footnotesize}
For exploring the classes and methods provided by the \Digraph module \footnote{See the \href{https://digraph3.readthedocs.io/en/latest/}{\Digraph documentation}} enter the Python3 commands following the session prompts marked with '\>\>\>' or '...' . The lines without the prompt are output from the Python3 interpreter.
...
In Listing \ref{list:1.1}  we import, for instance, from the \texttt{randomDigraphs}\index{randomDigraphs@\texttt{randomDigraphs}} module the \texttt{RandomDigraph} class \index{randomDigraphs@\texttt{randomDigraphs}!RandomDigraph@\texttt{RandomDigraph}} in order to generate a random digraph object $dg$ of order 5 - number of nodes called (decision) actions - and arc probability of $50\%$. The resulting digraph of order 5 and size -- number of arcs-- 12 is completely determined.
...
All digraphs instances are of \texttt{Digraph}\index{digraphs@\texttt{digraphs}|Digraph@\texttt{Digraph}} type and contain at least the following attributes (see Listing \ref{list:1.1}  Lines 11-12):
\begin{enumerate}
...
Different \texttt{show...()} methods like the \texttt{showShort()} method \index{digraphs@\texttt{digraphs}|Digraph@\texttt{Digraph}|showSort@\texttt{showShort()}} reveal us now that $dg$ is a crisp, irreflexive and connected digraph of order five (see Listing \ref{list:1.3}  Lines 1, 16, 26, 37 below).
...
These show methods usually rely upon corresponding compute methods, like:
\begin{itemize}
...
\end{itemize}
(see Listing \ref{list:1.6} Lines 5,7,16).
...
>>> dg.computeTransitivityDegree(InPercents=True)
 Decimal('60.00')
\end{lstlisting}
Mind that \texttt{showxxx()} methods output their results in the \emph{Python} console. We provide also some \texttt{showHTMLxxx()} methods which output their results in a system browser’s window.
