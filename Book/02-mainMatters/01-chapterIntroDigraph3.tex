...
All digraphs\index{digraphs@\texttt{digraphs}} instances are of \texttt{Digraph}\index{digraphs!Digraph@\texttt{Digraph}} type and contain at least the following attributes (see Listing \ref{list:1.1}  Lines 11-12):
\begin{enumerate}
...
Different \texttt{show...()} methods like the \texttt{showShort()} method \index{digraphs!Digraph!showSort@\texttt{showShort()}} reveal us now that $dg$ is a crisp, irreflexive and connected digraph of order five (see Listing \ref{list:1.3}  Lines 1, 16, 26, 37 below).
...
\begin{itemize}
\item The \texttt{computeSize()}\index{digraphs!Digraph!computeSize@\texttt{computeSize()}},
\item The \texttt{computeDeterminateness()}\index{digraph!Digrap!computeDeterminateness@\texttt{computeDeterminateness()}}, or
\item The \texttt{computeTransitivityDegree()}\index{digraphs!DigraphcomputeTransitivityDegree@\texttt{computeTransitivityDegree()}} method,
\end{itemize}
