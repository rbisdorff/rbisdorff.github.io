...
We are starting the tutorial with generating a uniformly random $[-1.0; +1.0]$-valued digraph of order 7, denoted $rdg$ and modelling, for instance, a binary relation $S(x,y)$ defined on the set of nodes of $rdg$. For this purpose, the \Digraph resources provide in {\tt randomDigraphs}\index{randomDigraphs@\texttt{randomDigraphs}} module a specific {\tt RandomValuationDigraph}\index{randomDigraphs!RandomValuationDigraph@\texttt{RandomValuationDigraph}} constructor.
 ...
 *------- Digraph instance description ------*
   ...
   Attributes          : ['name','actions','order',
                          ...
                          'gamma','notGamma']
\end{lstlisting}   
With the {\tt save()} \index{digraphs!Digraph!save@\texttt{save()}} method (see Listing \ref{list:2.1} Line 3) we may keep a backup version for future use of $rdg$ which will be stored in a file called {\tt tutRandValDigraph.py} in the current working directory.
...
The {\tt Digraph} class provides some generic {\tt showxxx()} methods for exploring the content of a given {\tt Digraph} object, like the \texttt{showRelationTable()}\index{digraphs!Digraph!showRelationTable@\texttt{showRelationTable()}}, the \texttt{showComponents()}\index{digraphs!Digraph!showComponents@\texttt{showComponents()}} and the \texttt{showNeighborhoods()}\index{digraphs!Digraph!showNeighborhoods@\texttt{showNeighborhoods()}} methods.
...
Here below, for illustration the source code of the {\tt relation} constructor of the {\tt AsymmetricPartialDigraph}\index{digraphs!AsymmetricPartialDigraph@\texttt{AsymmetricPartialDigraph}} class.
...
We may also extract the border --the part of a digraph induced by the union of its initial and terminal prekernels (see Chapter \ref{sec:17})--  as well as, the inner part -the complement of the border- with the help of two corresponding class constructors: \texttt{GraphBorder}\index{digraphs!GraphBorder@\texttt{GraphBorder}} and \texttt{GraphInner}\index{digraphs!|GraphInner@\texttt{GraphInner}} (see Fig. \ref{fig:2.3}).
