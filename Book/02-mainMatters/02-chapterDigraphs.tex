...
We may now extract both the \emph{}\emph{symmetric} as well as the \emph{asymmetric} part of digraph \texttt{rdg} with the help of two corresponding constructors (see Listing~\vref{list:2.3}) \index{AsymmetricPartialDigraph@\texttt{AsymmetricPartialDigraph} class}\index{SymmetricPartialDigraph@\texttt{SymmetricPartialDigraph} class}.
...
We may also extract the \emph{border} --the part of a digraph induced by the union of its initial and terminal prekernels (see Chapter~\ref{sec:17})--  as well as, the \emph{inner part} --the complement of the border-- with the help of two corresponding class constructors: \texttt{GraphBorder}\index{GraphBorder@\texttt{GraphBorder} class} and \texttt{GraphInner}\index{GraphInner@\texttt{GraphInner} class} (see Fig.~\vref{fig:2.3}).
...
It is \emph{D. Bouyssou} \index{Bouyssou@\emph{D. Bouyssou}} who first suggested us end of the nineties, when starting to work on the computation of digraph kernels with finite domain constraint solvers, that the $50\%$ criteria significance majority was a special value to be taken especially into account. The converging solution vectors of the fixpoint kernel equations confirmed this intuition.

These early observations led to the seminal articles on bipolar-valued epistemic logic and, where we introduced split truth/falseness semantics for a multi-valued logical processing of fuzzy preference modelling \citep{BIS-2000,BIS-2002,BIS-2004a}. The characteristic valuation domain still remains however the classical $[0.0;1.0]$ valuation domain.

It is again \emph{D. Boussyou}, when commenting a draft of our article with \emph{M. Pirlot} and \emph{M. Roubens} on kernels and choices in bipolar-valued digraphs \citep*{BIS-2006a}, who suggested to definitely switch to a $[-1.0;+1.0]$ valuation domain. Identifying truth and falseness immediately by the sign of the characteristic values revealed itself to be very efficient from a computational point of view. 

The new formalism opened the way to the important concept of disjunctive epistemic fusion which allowed us to design a logically and epistemically sound concept of bipolar-valued outranking digraph (see next Chap.~\ref{sec:3}). 
