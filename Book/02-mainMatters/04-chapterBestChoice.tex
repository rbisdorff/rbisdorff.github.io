...
The digraph put forward by D. Bouyssou in the October 2005 discussion is not weakly complete --node \texttt{a} is not outranking node \texttt{b} and vice versa-- does hence not represent, in our present sense, a valid outranking digraph instance. Yet, it may be a partial tournament and as such it is a strict outranking digraph, i.e. the asymmetric part --the codual-- of a valid outranking digraph. In this case, nodes \texttt{a} and \texttt{d} --the kernel of the strict outranking digraph-- would actually for sure outrank each other and, hence, represent both the natural best choice recommendation. However, in this codual digraph, node \texttt{a} becomes a \Condorcet winner --outranking for sure all other nodes-- and becomes the evident unique best choice recommendation.
