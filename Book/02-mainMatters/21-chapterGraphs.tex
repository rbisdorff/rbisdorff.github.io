...
With the \texttt{Q\_Coloring} class\index{Q\_Coloring@\texttt{Q\_Coloring} class}, a 3-coloring of the tutorial graph \texttt{g} may for instance be computed with a Gibbs sampler\index{Gibbs sampler} \citep{GEM-1984} and plotted as follows.
\begin{lstlisting}
...
Special classes of graphs, like $n \times m$ rectangular or triangular grids (\texttt{GridGraph}\index{GridGraph@\texttt{GridGraph} class} and \texttt{IsingModel}\index{IsingModel@\texttt{IsingModel} class}) are available from the \texttt{graphs} module. For instance, we may use a \emph{Gibbs} sampler\footnote{\citet{GEM-1984}} again for simulating an \emph{Ising Model} on such a grid \citep{ISI-1925}.
\begin{lstlisting}
...
Finally, we provide the \texttt{MetropolisChain} class, a specialization of the \texttt{Graph} class, for implementing a generic \emph{Metropolis} Monte Carlo Markov Chain (MCMC) sampler for simulating random walks on a given graph \citep{MET-1953}.
\begin{lstlisting}
...
Such $n$-cycle graphs are also provided as undirected graph instances by the \texttt{CycleGraph} class. \index{CycleGraph@\texttt{CycleGraph} class}
\begin{lstlisting}
...
\end{lstlisting}
The corresponding group stabilizers' sizes and frequencies: orbit 1 with 6 symmetry axes, orbit 2 with 4 symmetry axes, and orbits 3 and 4 both with one symmetry axis (see Lines 12-13 above), are illustrated in the corresponding unlabelled graphs of Fig.~\ref{fig:21.7.2}.
\begin{figure}[h]
\sidecaption[t]
