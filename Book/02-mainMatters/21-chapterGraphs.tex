...
In the \Digraph \texttt{graphs} module\index{graphs@\texttt{graphs} module}, the root \texttt{Graph} class \index{Graph@\texttt{Graph} class} provides a generic simple graph model, without loops and multiple links. A given object of this class contains at least the following attributes:
...
The stored graph instance may be reloaded and plotted with the generic \texttt{export\-GraphViz()} method (see Fig.~\vref{fig:21.1}):
\begin{lstlisting}
