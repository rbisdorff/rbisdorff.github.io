...
Suppose the file \texttt{berge.py} Footnote[18] contains the following \texttt{Graph} instance data:
\begin{lstlisting}
  vertices = {
...
We will first \emph{fuse} both $og$ and $ogd$ orientations above with an \emph{epistemic disjunction} operated with the symmetric \texttt{o-max} operator Footnote[x].
\begin{lstlisting}
