...
From graph theory we know that time interval intersections graphs must in fact be interval graph instances, i.e. \emph{triangulated} and \emph{co-comparative} graphs. The testimonies graph shown in Figure~\vref{fig:23.2} should therefore not contain any chordless cycle of four and more vertices. Now, the presence or not of such chordless cycles in the testimonies graph can be checked with the \texttt{computeChordlessCycles()} method \citep{BIS-2010}.\index{computeChordlessCycles@\texttt{computeChordlessCycles()}}
\begin{lstlisting}
...
By using colour sorting queues, the minimal vertex colouring shown in Figure~\vref{fig:23.5} is computable in $O\big(n log(n)\big)$ \citep{GOL-2004}.\index{computeMinimalVertexColoring@\texttt{computeMinimalVertexColoring()}}
\begin{lstlisting}
...
The resulting orientation of the edges of \texttt{g}, shown in Figure~\vref{fig:23.9}, is indeed transitive. The same procedure applied to the dual graph \texttt{-g} gives as well a transitive orientation of its edges shown in Figure~\vref{fig:23.10}.
...
We will first \emph{fuse}\index{FusionDigraph@\texttt{FusionDigraph} class} both $og$ and $ogd$ orientations above with an \emph{epistemic disjunction} operated with the symmetric \texttt{o-max} operator\index{o-max@\texttt{o-max}()} (see Sec.~\ref{sec:2.5}).
\begin{lstlisting}
...
Let us finally check in Figure~\vref{fig:23.11} that the two permutations [2, 3, 4, 8, 6, 1, 7, 5] and [4, 2, 8, 3, 1, 5, 6, 7], observed above, will generate corresponding \emph{isomorphic} permutation graphs.
\begin{figure}[ht]
  % \sidecaption
...
%\input{02-mainMatters/23-chapterPerfectGraphs.bbl}
