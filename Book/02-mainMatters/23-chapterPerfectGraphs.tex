...
\noindent Following Martin Golumbic (see \citet[p. 149]{GOL-2004})\index{Golumbic@\emph{M.Ch. Golumbic}}, we call a given graph \texttt{g}:
\begin{itemize}
...
By using color sorting queues, the minimal vertex coloring shown in Figure~\vref{fig:23.5} is computable in $O\big(n log(n)\big)$ \citep{GOL-2004}.\index{computeMinimalVertexColoring@\texttt{computeMinimalVertexColoring()}}
\begin{lstlisting}
