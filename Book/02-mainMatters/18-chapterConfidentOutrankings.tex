\chapter[On confident outrankings]{On confident outrankings with uncertain criteria significances}
\label{sec:18}
...
\section{Bipolar-valued likelihood of ``\emph{at least as good as}'' situations}
\label{sec:18.2}
...
A \emph{Monte Carlo} simulation with $10\,000$ runs empirically confirms the effective convergence to a Gaussian (see Fig. \ref{fig:18.3} realised with \emph{gretl} Footnote[4]).
\begin{figure}[h]
%\sidecaption
...
\section{Confidence level of outranking situations}
\label{sec:18.3}
