...
\begin{figure}[h]
%\sidecaption
\includegraphics[width=12cm]{Figures/18-1-weightDistributions.png}
\caption{Four models of uncertain criteria significance weights}
\label{fig:18.1}       % Give a unique label
\end{figure}
It is worthwhile noticing in Fig.~\ref{fig:18.1} that these four uncertainty models all admit the same expected value, $E(W_j)$, however, with a respective variance which goes decreasing from $1/3$, to $1/9$ of the square of $E(W_j)$ \citep{BIS-2014}.
...
\begin{itemize}
\item $+1$ signifies $x$ is at least as well assessed as $y$ on criterion $j$,
\item $-1$ signifies that $x$ is not at least as well assessed as $y$ on criterion $j$,
\item $0$ signifies that it is unclear whether, on criterion $j$, $x$ is at least as well assessed as $y$.
\end{itemize}
\begin{figure}[h]
%\sidecaption
\includegraphics[width=11cm]{Figures/18-2-rCharacteristic.pdf}
\caption{Bipolar-valued outranking characteristic function}
\label{fig:18.2}       % Give a unique label
\end{figure}
Each criterion $j$ in $F$ contributes the random significance $W_j$ of his ``\emph{at least as well assessed as}'' characteristic $r(x \geq_j y)$ to the global characteristic $\tilde{r}(x \geq y)$ in the following way:
\begin{equation}
      \tilde{r}(x \geq y) \; = \; \sum_{j \in F} W_j \times r(x \geq_j y) )
    \end{equation}
Thus, $\tilde{r}(x \geq y)$ becomes a simple sum of positive or negative independent random variables with known means and variances where $\tilde{r}(x \geq y) \, > \, 0$ signifies $x$ is globally at least as well assessed as $y$, $\tilde{r}(x \geq y) \, < \, 0$ signifies that $x$ is not globally at least as well assessed as $y$, and $\tilde{r}(x \geq y)\,=\,0$ signifies that it is unclear whether $x$ is globally at least as well assessed as $y$.
...
\end{eqnarray}
And the \emph{likelihood of validation}, respectively \emph{invalidation} of an ``\emph{at least as well assessed as}'' situation, denoted $lh(x \geq y)$,  may hence be assessed by the probability $P(Y > 0) = 1.0 - P(Y \leq 0)$ that $Y$ takes a positive, resp. $P(Y < 0)$ takes a negative value. In the bipolar-valued case here, we can judiciously make usage of the standard Gaussian \emph{error function} , i.e. the bipolar $2P(Z) - 1.0$ version of the standard Gaussian $P(Z)$ probability distribution function \citep{NR3-2007-6}:
\begin{equation}
  lh(x \geq y) \;=\; -\text{erf}\big(\frac{1}{\sqrt{2}}\frac{-E(Y)}{\sqrt{V(Y)}} \big)
\end{equation}
The range of the bipolar-valued $lh(x \geq y)$ hence becomes $[-1.0;+1.0]$, and $-lh(x \geq y) \,=\, lh(x \not\geq y)$ , i.e. a \emph{negative likelihood} represents the likelihood of the correspondent \emph{negated} ``\emph{at least as well assessd as}'' situation. A likelihood of $+1.0$ (resp. $-1.0$) means the corresponding preferential situation appears \emph{certainly} validated (resp. invalidated).

\begin{example} Let $x$ and $y$ be evaluated wrt 7 equisignificant criteria; Four criteria positively support that $x$ is \emph{as least as  well assessed as} $y$ and three criteria support that $x$ is \emph{not at least as well assessed as} $y$. Suppose $E(W_j) = w$ for $j = 1,...,7$ and $W_j \leadsto Tr(0, 2w, w)$ for $j = 1,...7$. The expected value of the global ``\emph{at least as well assessed as}'' characteristic value becomes: $E\big(\tilde{r}(x \geq y)\big)\, = \, 4w - 3w = w$ with a variance $V\big(\tilde{r}(x \geq y)\big)\,=\, 7\frac{1}{6}w^2$. 

If $w = 1$, $E\big(\tilde{r}(x \geq y)\big)\, = \, 1$ and $sd\big(\tilde{r}(x \geq y)\big)\,=\, 1.08$. By the CLT, the bipolar likelihood of the \emph{at least as well assessed as} situation becomes: $lh(x \geq y)\,=\, 0.66$, which corresponds to a global support of $(0.66 + 1.0)/2 = 83\%$ of the criteria significance weights.
...
Following the classical outranking approach (see \citep{BIS-2013}), we say, from an epistemic perspective, that decision action $x$ \emph{outranks} decision action $y$ at \emph{confidence} level $\alpha\%$, when:
\begin{itemize}[topsep=1pt]
\item An expected majority of criteria validates, at confidence level $\alpha\%$ or higher, a global ``\emph{at least as well assessed as}'' situation between $x$ and $y$, and
...
\item An expected minority only of criteria at confidence level $-\alpha\%$ or lesser, validates a global ``\emph{at least as well assessed as}'' situation between $x$ and $y$, and
...
For the corresponding confident outranking digraph, we assume uncerain criteria significance weights of \emph{triangular} ditribution and require a confidence level of $\alpha = 90\%$.
...
Fig.~\ref{fig:18.4} shows the corresponding strict $90\%$-confident outranking digraph, oriented by its initial and terminal strict prekernels.
\begin{lstlisting}
>>> gcd90 = ~ (-g90)
...
\end{lstlisting}
At $99\%$ confidence, the minimal required significance majority support amounts to $64.3\%$ (see Listing~\ref{list:18.4} Line 25 above). As a result, most outranking situations don't get anymore validated, like the outranking situations between action \texttt{a1} and actions \texttt{a3}, \texttt{a4}, \texttt{a5} and \texttt{a6} (see Line 5 above). The overall epistemic determination of the digraph consequently drops from $.1\%$ to $56.6\%$ (see Line 25).
...
\begin{lstlisting}[caption={$90\%$-confident outranking relation with uniform variates},label=list:18.5, basicstyle=\ttfamily\scriptsize]
...
>>> gu90.showRelationTable()
  * ---- Outranking Relation Table -----
  ...
  -------|-------------------------------------------------------
    ...
    Determinateness    : 0.24 (62.1%)
\end{lstlisting}
Despite lower likelihood values (see the $g90$ relation in Listing~table above), we keep the same confident majority level of $57.1\%$ (see Listing~\ref{list:18.5} Line 25 above) and, hence, also the same $90\%$-confident outranking digraph.
