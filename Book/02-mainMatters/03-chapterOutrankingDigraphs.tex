...
\begin{quotation}
“On what principles may we base the arcs to be added to [the unanimous dominance digraph] in order to deduce a digraph which is both – in best possible concordance with the [given] multiple [preferential] points of view and – not too poor in comparable pairs [of decision actions]?” \citep[p.6]{ROY-1966}\index{Roy}
\end{quotation}
\vspace{1cm}
...
The seminal work on outranking digraphs goes back to the seventies and eighties (\citet{ROY-1985,ROY-1991} and \citet*{ROY-1993}). The LAMSADE \footnote{Laboratoire d’Analyse et de Modélisation de Systèmes pour l’Aide à la Décision (LAMSADE), University Paris Dauphine, founded by B. Roy} was indeed a major site in the development of the outranking  approach to multiple criteria decision making.

The ongoing success of the original outranking concept stems from the fact that it is rooted in a sound pragamatism. The multiple criteria performance tableau, necessarily associated with a given outranking digraph, is indeed convincingly objective and meaningful. And, ideas from social choice theory gave initially the insight that a pairwise voting mechanism à la \Condorcet could provide an order-statistical tool for aggregating a set of preference points of view into what M. Barbut calls the \emph{central} \Condorcet point of view \cite{BAR-1966}; in fact the median of the multiple preference points of view, at minimal absolute \Kendall's ordinal correlation $\tau$ measure from all individual points of view (see Chapter~\ref{sec:16}).

Considering thus each performance criterion as a subset of unanimous voters and arithmetically balancing the votes in favour against considerable counter-performances in disfavour gave eventually rise to the modern concept of \emph{outranking situation}, a distinctive feature of the Multiple Criteria Decision Aid approach \citep{ROY-1991}.  A modern definition would be : An alternative $x$ is said to \emph{outrank} alternative $y$ if and only if – there exists a \emph{significant majority} of criteria who confirm that alternative $x$ is to be considered as \emph{at least as worth as} alternative $y$ (the concordance argument); and – no discordant criterion opens to significant doubt the validity of the previous confirmation by revealing a considerable counter-performance of alternative $x$ compared to $y$ (the discordance argument).

If the concordance argument was always well recieved, the discordance argument, very confused in the beginning \citep{ROY-1966}, could only be handled in an epistemically correct and logically sound way when we used a bipolar disjunctive fusion operator for polarising the concordance with the discordance argument \citep{BIS-2013}.
