...
\caption{The fifteen academic subjects taken into account for assessing the student enrolment quality}
...
The resulting ranking of the 5 Universities including the lower-closed quintile score limits may be nicely illustrated  with the \texttt{showHTMLRatingHeatmap()} method.\index{showHTMLRatingHeatmap@\texttt{showHTMLRatingHeatmap()}}
\begin{lstlisting}
...
In Listing~\vref{list:14.5}, the \texttt{showRankingConsensusQuality()} method\index{showRankingConsensusQuality@\texttt{showRankingConsensus\-Quality()}} reveals finally how fair \Kemeny and \Copeland rules actually balance the fifteen academic subjects. 
