...
A corresponding \emph{graphviz} drawing with the special \texttt{exportRatingByRankingGraph\-Viz()} method\index{exportRatingByRankingGraphViz@\texttt{exportRatingByRankingGraphViz()}} may well illustrate in Figure~\vref{fig:14.4} these enrolment quality equivalence classes.
\begin{lstlisting}
...
\begin{figure}[ht]
\sidecaption[t]
\includegraphics[width=6cm]{Figures/14-4-ratingResult.pdf}
...
Indeed, there appears an outranking circuit among the three universities \texttt{U1}, \texttt{U2} and \texttt{U4} rated into the 2nd quintile. It is, hence, interesting, to verify if the epistemic fusion of the rating-by-ranking results, one may obtain when applying two different ranking rules, like the \Copeland and \NetFlows rules, does actually confirm our rating-by-ranking result shown in Figure~\vref{fig:14.4}. For this purpose we make usage of the \texttt{RankingsFusionDigraph} class \index{RankingsFusionDigraph@\texttt{RankingsFusionDigraph} class} (see Lines 5-8 in List.~\vref{list:14.4}).
...
...                   pq,pt,rankingRule='Copeland')
>>> lqr1 = LearnedQuantilesRatingDigraph(\
...                   pq,pt,rankingRule='NetFlows')
...
\begin{figure}[ht]
\sidecaption[t]
\includegraphics[width=5cm]{Figures/14-5-fusionResult.pdf}
\caption{Disjunctive fusion of the \Copeland and the \NetFlows rankings}
\label{fig:14.5}       % Give a unique label
\end{figure}

Figure~\vref{fig:14.5} reveals that the \Copeland and \NetFlows rules diverge solely in rating University \texttt{U3}. When the \Copeland rule rates \texttt{U3} in the first quintile ($[0.00- 0.20[$), the \NetFlows puts \texttt{U3} togeter with \texttt{U1}, \texttt{U2} and \texttt{U4} into the second quintile ($[0.20- 0.40[$).  

To eventually check the quality of our \Copeland \texttt{rating-by-ranking} result, we shall below compute a direct sorting into the historic 2004 quintiles of the enrolment quality scores, without using any outranking digraph based ranking rule.
...
The \texttt{showSortingCharacteristics()} method\index{showSortingCharacteristics@\texttt{showSortingCharacteristics()}} gives a precise look in Listing~\vref{list:14.5} on these quintiles sorting characteristics.
\begin{lstlisting}[caption={Showing quantiles sorting characteristics},label=list:14.5,basicstyle=\ttfamily\scriptsize]
...
\begin{lstlisting}[caption={Showing a quintiles rating-by-sorting result},label=list:14.6]
...
The quintiles rating-by-sorting result, shown in Listing~\vref{list:14.6}, confirms so the previous \Copeland rating-by-ranking result. However, University \texttt{U5} is here sorted conjointly into the fourth and the fifth quintile, and as such would is part of the top rated institutions. A result already convincingly illustrated in the ranked heatmap shown in Figure~\vref{fig:14.3}. 

\vspace{1cm}

Let us conclude this hypothetical rating cae study, by saying that we prefer this latter rating-by-sorting approach; perhaps impreciser, due the case given, to missing and contradictory performance data; yet, well grounded in a powerful bipolar-valued logical and espistemic framework.
