\chapter{Bipolar-valued relational equivalence}
\label{sec:16}

\abstract*{ Starting from a motivating data about movie critics' star rating, we show that \Kendall 's ordinal correlation index tau may be extended to a bipolar-valued relational equivalence characteristic of  bipolar-valued digraphs. This finding gives way, on the one hand, to measure the fitness and fairness of our ranking heuristics. On the other hand, it provides a tool for illustrating decision objectives and criteria preference divergences.}
...
Let us consider such a performance tableau in file \texttt{graffiti07.py} \footnote{The file \texttt{graffiti07.py} may be found in the \texttt{examples} directory of the \Digraph resources.} gathering a Movie Magazine's rating of some movies that could actually be seen in town (see Fig.~\vref{fig:16.1}, \emph{Graffiti}, Edition Revue Luxembourg, September 2007, p. 30.) \footnote{ The data file \texttt{graffiti07.py} (\texttt{PerformanceTableau} format) may be found in the \texttt{examples} directory of the \Digraph resources.}.
\begin{lstlisting}
...
\end{figure}
There is no doubt that movie \texttt{mv\_QS}, with $6$ \emph{must be seen} marks, is correctly best-ranked and the movie \texttt{mv\_TV} is worst-ranked with five \emph{don't like} marks.

\section{Modelling pairwise bipolar-valued rating opinions}
\label{sec:16.2}

Let us explicitly construct the underlying bipolar-valued outranking digraph and consult in Fig.~\vref{fig:16.3} the pairwise characteristic values we obtain between the two best-ranked movies, namely \texttt{mv\_QS} and \texttt{mv\_RR} with the \texttt{showHTMLPairwiseOutrankings} method.\index{showHTMLPairwiseOutrankings@\texttt{showHTMLPairwiseOutrankings()}}
\begin{lstlisting}[caption={Showing pairwise outrankings in a browser view},label=list:16.1]
>>> from outrankingDigraphs import\
...
The complete table of pairwise significance majority margins of global ``\emph{at least as well rated as}'' opinions, ranked by the same rule as shown in the heatmap in Fig.~\vref{fig:16.3}, may be shown in Fig.~\vref{fig:16.4} with the help of the \texttt{showHTMLRelationTable()} method\index{showHTMLRelationTable@\texttt{showHTMLRelationTable()}}. 
\begin{lstlisting}[caption={Showing the outranking characteristics, ranked by the \NetFlows rule in a browser view},label=list:16.2]      
...
In the next Chapter \vref{sec:17}, we thouroughly discuss the computation of kernels in bipolar-valued digraphs.
