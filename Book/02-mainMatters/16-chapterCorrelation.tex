...
Such a kind of preordering of the movies can, for instance, be computed in Listing~\vref{list:16.11} with the \texttt{RankingByChoosingDigraph} class\index{RankingByChoosingDigraph@\texttt{RankingByChoosingDigraph} class} from the \texttt{transitiveDigraphs} module\index{transitiveDigraphs@\texttt{transitiveDigraphs} module}, where we iteratively extract best remaining choices --initial kernels-- and worst remaining choices --terminal kernels (see Chapter~\ref{sec:17}).
