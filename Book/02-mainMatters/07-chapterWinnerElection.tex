...
>>> lvp.linearBallot
    {'v1': ['c2', 'c1', 'c3',],
     ...
     'v5': ['c3', 'c1', 'c2',]} 
 \end{lstlisting}
Notice that in this random example, the five voters are weighted (see Listing~\vref{list:7.1} Line 6-7). Their linear ballots can be viewed with the \texttt{showLinearBallots}\index{showLinearBallots@\texttt{showLinearBallots()}} method.
\begin{lstlisting}
...
>>> lvp.computeInstantRunoffWinner(Comments=True)
 ...
 \end{lstlisting}
In stage 1, no candidate obtains an absolute majority of votes. Candidate \texttt{a2} obtains the minimal number of votes (2/15) and is, hence, eliminated. In stage 2, candidate \texttt{c1} obtains an absolute majority of the votes (8/15) and is eventually elected (see Listing~\vref{list:7.2}).
...
>>> lvp.computeBordaWinners()
 ['c1']
\end{lstlisting}
Candidate \texttt{c1} obtains the minimal \Borda score, followed by candidate \texttt{c3} and finally candidate \texttt{c2} (see Listing~\vref{list:7.3}). The corresponding \Borda \emph{rank analysis table} may be printed out with a corresponding \texttt{showRankAnalysisTable()} command \index{showRankAnalysisTable@\texttt{showRankAnalysisTable()}}.
...
In our randomly generated election results, we are lucky: Candidate \texttt{c1} is both the instant runoff winner and the \emph{Borda} winner (see Listings~\vref{list:7.2} and \vref{list:7.3}). However, we could also follow the \emph{Marquis de Condorcet}'s advice, and compute the \emph{majority margins} obtained by voting for each individual pair of candidates.
